Feature: Consultar OP - vista por FECHA - Operación: Anular OP
              Yo como usuario quiero entrar al sistema Gestión - Sistema Apoyo Egresos - Gestión de Ordenes, poder consultar ordenes de pago en la vista por Fecha y acción Anular OP
...
Scenario: Ir a la opción Ordenes de Pago por FECHA desde Gestión de Ordenes
    Given que se cliquea el elemento "a" con el texto " Gestión de Órdenes "
        ...
        When se espera "2" segundos
#Operación Anular OP - Concepto FPE REEMBOLSO CANDIDATO
            And que en el elemento "input" con id "fechaDesde" se ingresó el texto "01-01-2023"
        When se espera "1" segundos
        ...
        When se espera "1" segundos
            And que se presionó la tecla "F"
        When se espera "1" segundos
            And que se presionó la tecla "ArrowDown"
            And que se presionó la tecla "ArrowDown"
            And que se presionó la tecla "ArrowDown"
        ...
        When se espera "3" segundos
            And que se encontró un elemento "#tblOrdenesPago > tbody > tr:nth-child(1) > td.text-center.sorting_1" con el texto "FPE REEMBOLSO CANDIDATO"
        When se espera "2" segundos