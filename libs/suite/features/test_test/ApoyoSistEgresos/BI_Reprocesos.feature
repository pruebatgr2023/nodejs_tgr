Feature: Reporte BI Reprocesos OP Renta
              Yo como usuario quiero entrar al sistema Gestión - Sistema Apoyo Egresos - Gestión de Ordenes y poder ver el reporte BI de Reprocesos OP Renta
...
Scenario: Ir al reporte BI de Ordenes de Pago Anuladas desde Gestión de Ordenes
    Given que se cliquea el elemento "a" con el texto " Gestión de Órdenes "
        When se espera "3" segundos
            And que se cliquea el elemento "a" con el texto "Ver Reprocesos OP Renta "
        When se espera "3" segundos 
            And que se cambió a la pestaña "2"
    Given que se encuentra en la url "http://bi-desa2019/Reports/report/Divisi%C3%B3n%20de%20Operaciones/Egresos/Reprocesos%20quiebra%20y%20egresos%20cuestionados%20de%20procesos%20de%20Renta"
        When se espera "5" segundos
            And que se encontró un elemento "#main > div > paginated-report-viewer > div > section > breadcrumbs > div > span:nth-child(4) > span.ng-binding.ng-scope" con el texto "Reprocesos quiebra y egresos cuestionados de procesos de Renta"