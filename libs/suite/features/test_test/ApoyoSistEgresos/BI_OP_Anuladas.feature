Feature: Reporte BI Ordenes de Pago Anuladas
              Yo como usuario quiero entrar al sistema Gestión - Sistema Apoyo Egresos - Gestión de Ordenes y poder ver el reporte BI de Ordenes de Pago Anuladas
...
Scenario: Ir al reporte BI de Ordenes de Pago Anuladas desde Gestión de Ordenes
    Given que se cliquea el elemento "a" con el texto " Gestión de Órdenes "
        When se espera "1" segundos
            And que se cliquea el elemento "a" con el texto "Ver Órdenes Anuladas "
        When se espera "2" segundos 
            And que se cambió a la pestaña "2"
    Given que se encuentra en la url "http://bi-desa2019/Reports/report/Divisi%C3%B3n%20de%20Operaciones/Egresos/Ordenes%20de%20Pago%20Anuladas%20onpremise"
        When se espera "5" segundos
            And que se encontró un elemento "#main > div > paginated-report-viewer > div > section > breadcrumbs > div > span:nth-child(4) > span.ng-binding.ng-scope" con el texto "Ordenes de Pago Anuladas onpremise"