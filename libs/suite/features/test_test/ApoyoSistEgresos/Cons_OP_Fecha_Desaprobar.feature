Feature: Consultar OP - vista por FECHA - Operación: Aprobar OP
              Yo como usuario quiero entrar al sistema Gestión - Sistema Apoyo Egresos - Gestión de Ordenes, poder consultar ordenes de pago en la vista por Fecha y acción Desaprobar OP
...
Scenario: Ir a la opción Ordenes de Pago por FECHA desde Gestión de Ordenes
    Given que se cliquea el elemento "a" con el texto " Gestión de Órdenes "
        ...
        When se espera "2" segundos
#Operación Desaprobar OP - Concepto RTA_PERIODICA_LIBRE_SENSE
            And que en el elemento "input" con id "fechaDesde" se ingresó el texto "01-07-2023"
        When se espera "1" segundos
            And que en el elemento "input" con id "fechaHasta" se ingresó el texto "30-08-2023"
            And que se presionó la tecla "Enter"
        When se espera "1" segundos
            And se cliquea el selector "#operacion"
        When se espera "1" segundos
            And que se presionó la tecla "ArrowDown"
        When se espera "1" segundos
            And que se presionó la tecla "Enter"
        ...
        When se espera "1" segundos
            And que se presionó la tecla "R"
        When se espera "1" segundos
            And que se presionó la tecla "Enter"
        When se espera "1" segundos
            And se cliquea el selector "#btnBuscar"
        When se espera "3" segundos
            And que se encontró un elemento "#tblOrdenesPago > tbody > tr:nth-child(1) > td.text-center.sorting_1" con el texto "RTA_PERIODICA_LIBRE_DE_SENCE"
        When se espera "2" segundos